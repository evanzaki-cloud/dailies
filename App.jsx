...
const SESSIONS = {
  upperA: { name: "Upper A", ex: [
    { id: "flatpress", name: "Flat dumbbell press", sets: 4, lo: 6, hi: 10, hint: "Machine chest press for the heavy sets once the 80s get easy." },
    { id: "latpulldown", name: "Lat pulldown, wide grip", sets: 4, lo: 8, hi: 12, plate: true, hint: "High pulley, lat bar. Pull to the collarbone, elbows down and back." },
    { id: "shoulderpress", name: "Seated dumbbell shoulder press", sets: 3, lo: 8, hi: 12 },
    { id: "cablerow", name: "Seated cable row", sets: 3, lo: 10, hi: 12, plate: true, hint: "Low pulley, handles. Chest up, squeeze the shoulder blades." },
    { id: "cablelateral", name: "Cable lateral raise", sets: 3, lo: 12, hi: 20, plate: true, each: true, hint: "Low pulley, single handle, cable across the body. Dumbbells if it's awkward." },
    { id: "pushdown", name: "Triceps pushdown", sets: 3, lo: 10, hi: 15, plate: true, hint: "High pulley, bar or rope. Elbows pinned." },
    { id: "cablecurl", name: "Cable curl", sets: 3, lo: 10, hi: 15, plate: true, hint: "Low pulley, bar." },
  ] },
  lowerA: { name: "Lower A", ex: [
    { id: "goblet", name: "Dumbbell squat, heels raised", sets: 4, lo: 8, hi: 12, hint: "Goblet with one dumbbell; one in each hand at your sides once that's heavy." },
    { id: "rdl", name: "Romanian deadlift", sets: 4, lo: 8, hi: 12, hint: "Dumbbells. Hinge, soft knees, bar along the thighs." },
    { id: "legext", name: "Leg extension", sets: 3, lo: 10, hi: 15, plate: true, hint: "Pause a second at the top." },
    { id: "legcurl", name: "Leg curl", sets: 3, lo: 10, hi: 15, plate: true, hint: "On the leg developer, lying or seated, whichever it does." },
    { id: "bss", name: "Bulgarian split squat", sets: 3, lo: 8, hi: 12, each: true },
    { id: "calf", name: "Standing calf raise", sets: 3, lo: 15, hi: 20 },
    { id: "cablecrunch", name: "Cable crunch", sets: 3, lo: 10, hi: 15, plate: true, hint: "Kneel facing the high pulley, rope or bar behind your head, crunch ribs to hips." },
    { id: "legraise", name: "Lying leg raise", sets: 3, lo: 10, hi: 15 },
  ] },
  upperB: { name: "Upper B", ex: [
    { id: "inclinepress", name: "Incline dumbbell press", sets: 4, lo: 6, hi: 10, hint: "Flat if the bench doesn't incline." },
    { id: "csrow", name: "Chest-supported dumbbell row", sets: 4, lo: 8, hi: 12, hint: "Chest on the incline bench." },
    { id: "underhandpull", name: "Lat pulldown, underhand", sets: 3, lo: 8, hi: 12, plate: true, hint: "Shoulder-width, palms facing you." },
    { id: "machinepress", name: "Machine chest press", sets: 3, lo: 8, hi: 12, plate: true, hint: "The press arm. Set the seat so the handles sit at mid-chest." },
    { id: "facepull", name: "Face pull", sets: 3, lo: 12, hi: 20, plate: true, hint: "High pulley, rope or two handles. Pull to the forehead, elbows high." },
    { id: "lateral", name: "Dumbbell lateral raise", sets: 3, lo: 12, hi: 20 },
  ...
  lowerB: { name: "Lower B", ex: [
    ...
    { id: "legcurl", name: "Leg curl", sets: 4, lo: 10, hi: 15, plate: true, hint: "Hamstrings lead today." },
    { id: "stepup", name: "Step-ups onto bench", sets: 3, lo: 10, hi: 12, each: true },
    { id: "legext", name: "Leg extension", sets: 3, lo: 12, hi: 15, plate: true },
    { id: "pullthrough", name: "Cable pull-through", sets: 3, lo: 12, hi: 15, plate: true, hint: "Low pulley, rope or a handle between the legs, hinge and squeeze the glutes. Single-leg RDL if you'd rather." },
    { id: "calf", name: "Standing calf raise", sets: 3, lo: 15, hi: 20 },
    { id: "cablecrunch", name: "Cable crunch", sets: 3, lo: 10, hi: 15, plate: true },
...
function suggest(ex, last, units) {
  ...
  if (ex.plate) {
    if (!last) return { w: "", text: `First time. Find a pin you can do for ${ex.lo} with a rep or two left, and log the plate number.` };
    const reps = last.reps.slice(0, ex.sets);
    const hitAll = reps.length >= ex.sets && reps.every((r) => r >= ex.hi);
    if (hitAll) return { w: last.w + 1, add: true, text: `You hit ${ex.hi} on every set at plate ${last.w}. Move the pin down one: plate ${last.w + 1}.` };
    return { w: last.w, text: `Last time: plate ${last.w} for ${last.reps.join(", ")}. Beat it.` };
  }
...
const defaultState = () => ({ v: 3, adapaleneStart: todayKey(), seeds: {}, threadingDone: false, dermDone: false, days: {}, trainingStart: null, units: "lb", workouts: [], active: null });
    ...
    <div className="preview">
      {ROTATION.map((id) => (
        <div key={id}>
          ...
          <p>{SESSIONS[id].ex.map((e) => `${e.name} ${e.sets}×${e.lo}–${e.hi}${e.time ? " s" : ""}${e.each ? " each side" : ""}${e.plate ? " (machine)" : ""}`).join(". ")}.</p>
        </div>
      ))}
      <p style={{ marginTop: 12 }}>Rotation: Upper A, Lower A, Upper B, Lower B, with a walk on the days between. Rest 2 minutes on the first two lifts, 60–90 seconds on the rest. Machine exercises are logged by plate number; the rest by dumbbell weight.</p>
    </div>
  );
}
...
const GUIDE = [
  ...
  { t: "Lifting", items: [
    "Kit: the multi-gym (lat pulldown, cable row, press arm, leg extension and curl, high and low pulleys), the 80 lb NÜOBELLs and the bench. That covers everything except barbell squats, deadlifts and pull-ups, so a commercial gym is optional until late in the build phase.",
    ...
    "Logging: dumbbell exercises by the weight per hand; machine exercises by the plate number the pin is in. The plates are probably 10 lb each but the pulleys change what reaches your hands, so the number only has to beat last time's number.",
    "Progression: when every set hits the top of its rep range, add weight next time: 5 lb on dumbbells, one plate on the machine. The Gym tab tells you. Otherwise beat last time's reps at the same load.",
    "Rest 2 minutes on the first two lifts of a session, 60–90 seconds after. Log every set as you finish it.",
    "When the 80s get easy on presses, do the heavy sets on the machine chest press and keep the dumbbells for the higher-rep work. Legs: extension and curl take the load off the dumbbells, and Bulgarian split squats with an 80 in each hand stay hard for a long time.",
    "Abs on both lower days: cable crunch is the main one (kneel facing the high pulley, rope or bar behind your head, crunch the ribs toward the hips, load it like any other lift), plus leg raises and the plank. Keep heavy oblique work minimal to keep the waist narrow.",
        ...
        <section>
          <div className="gymhead">
            <div>
              ...
              <div className="hint">Type reps as you finish each set. Dumbbells by weight per hand, machine by plate number.</div>
            </div>
          ...
          {SESSIONS[active.session].ex.map((e, idx) => {
            ...
            return (
              <div key={e.id} className="exercise">
                ...
                {e.hint && <div className="hint" style={{ marginTop: 2 }}>{e.hint}</div>}
                <div className={"suggest" + (sug.add ? " add" : "")}>{sug.text}</div>
                <div className="inputs">
                  <input className="winput" type="number" inputMode={e.plate ? "numeric" : "decimal"} step={e.plate ? "1" : "0.5"} placeholder={e.plate ? "pin" : "0"} value={rec.w} onChange={(ev) => setW(e.id, ev.target.value)} aria-label={`${e.name} ${e.plate ? "plate" : "weight"}`} />
                  <span className="unit">{e.plate ? "plate" : units}</span>
                  <div className="sets">
                    {rec.reps.map((r, i) => (
